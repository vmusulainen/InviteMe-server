...
Accounts.validateLoginAttempt(function(options) {
    ...
    // more complex.
    if ((options.user.username === "admin") || ((options.user.username === "customer"))) {
        return true;
    }
    if (options.user.emails[0].verified === true) {
        return true;
    } else {
        throw new Meteor.Error("email-not-verified", "You must verify your email address before you can sign in");
    }
});
